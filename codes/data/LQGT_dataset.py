...
class LQGTDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.opt["data_type"] == "lmdb":         #（不需要）
            if (self.GT_env is None) or (self.LR_env is None):
                self._init_lmdb()

        GT_path, LR_path = None, None           #（需要）
        ...
        if self.opt["data_type"] == "lmdb":         #（不需要）
            ...
            resolution = None                 
        img_GT =  cv2.imread(GT_path)      #图片读取
        # img_GT = scipy.io.loadmat(GT_path)['sensor_data'] #读取mat文件
        img_GT = img_GT.transpose(2,0,1)[0]
        img_GT = img_GT[None,...]
        img_GT = np.float32(img_GT)/255         
        # print("-------------------------------------------------------------",img_GT.shape)         
        ...
        if self.LR_paths:  # LR exist
            ...
            if self.opt["data_type"] == "lmdb":
                ...
                resolution = None
            img_LR =  cv2.imread(LR_path)       #图片读取
            # img_LR = scipy.io.loadmat(LR_path)['sensor_data'] #读取mat文件
            img_LR  = img_LR.transpose(2,0,1)[0]
            img_LR = img_LR[None,...]
            img_LR = np.float32(img_LR)/255
            # img_LR = util.read_img(self.LR_env, LR_path, resolution)
            # print("*******************************************",img_GT.shape)
            ##########################################################################  
        else:  # down-sampling on-the-fly（没用）
        ...
        # print("-------------------------------------------------------------",img_GT.shape,img_LR.shape)
