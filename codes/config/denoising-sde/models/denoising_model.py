...
class DenoisingModel(BaseModel):
    ...
    def optimize_parameters(self, step, timesteps, sde=None):
        ...
        ######################尝试可视化学习到的噪声，生成相应的文件并保存

        noise_image = noise.squeeze().cpu().detach().numpy()
        noise_image = noise_image[0]  # 选择第一个通道
        # 设置图像保存路径
        save_dir = "/home/b110/code/LZL/irsde/codes/config/deblurring/sde_noise/"
        ...
        if image_count < 1000:
           ...
           # 保存图像
           save_path = os.path.join(save_dir, f"{image_count + 1}.png")
           plt.savefig(save_path)
           plt.close()

        ######################尝试可视化学习到的噪声和分数，生成相应的文件并保存
