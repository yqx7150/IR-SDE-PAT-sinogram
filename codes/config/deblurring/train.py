...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            # print("ccccccccccccccc",LQ.shape,LQ.dtype,GT.shape,GT.dtype)
            print("训练次数：{},剩余次数：{}".format(current_step,400001-current_step))
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                # tensorboard logger
                if opt["use_tb_logger"] and "debug" not in opt["name"]:
                    tb_logger.add_scalar("psnr", avg_psnr, current_step)      #可以添加参数，比如SSIM
            ...
            if current_step % opt["logger"]["save_checkpoint_freq"] == 0:
                if rank <= 0:
                    ...
                    model.save_training_state(epoch, current_step)               #保存训练状态，可删除
...
# 打印运行时间
print(f"代码执行时间为：{hours} 小时 {minutes} 分钟 {seconds} 秒")